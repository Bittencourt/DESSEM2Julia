...
@testset "Network Topology Parser Tests" begin
    ...
    @testset "NetworkTopology Type" begin
        buses = [NetworkBus(bus_number = 1), NetworkBus(bus_number = 2)]
        lines = [NetworkLine(from_bus = 1, to_bus = 2, circuit = 1)]

        topology =
            NetworkTopology(buses = buses, lines = lines, stage = 1, load_level = "LEVE")
    ...
    @testset "ONS PDO_SOMFLUX Parsing" begin
        ...
        if isfile(somflux_file)
            ...
            line_1027_556 =
                findfirst(l -> l.from_bus == 1027 && l.to_bus == 556, topology.lines)
            ...
            line_6442_3050 =
                findfirst(l -> l.from_bus == 6442 && l.to_bus == 3050, topology.lines)
    ...
    @testset "ONS Complete Topology Parsing" begin
        ...
        if isdir(ons_case)
            ...
            println(
                "  ✓ Complete topology: $(length(topology.buses)) buses, $(length(topology.lines)) lines",
            )
        ...
        if isdir(ons_case)
            ...
            for bus in topology.buses
                if !isempty(bus.subsystem)
                    subsystem_counts[bus.subsystem] =
                        get(subsystem_counts, bus.subsystem, 0) + 1
                end
            end
