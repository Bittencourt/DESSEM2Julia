...
@testset "ONS Sample Integration Tests" begin
    ...
    @testset "ONS TERMDAT.DAT" begin
        ...
        if isfile(termdat_path)
            ...
            println(
                "  ✅ ONS TERMDAT: $(length(data.plants)) plants, $(length(data.units)) units",
            )
        else
    ...
    @testset "ONS ENTDADOS.DAT" begin
        ...
        if isfile(entdados_path)
            ...
            println(
                "  ✅ ONS ENTDADOS: $(length(data.time_periods)) periods, " *
    ...
    @testset "ONS OPERUT.DAT" begin
        ...
        if isfile(operut_path)
            ...
            println(
                "  ✅ ONS OPERUT: $(length(data.init_records)) INIT records " *
    ...
    @testset "ONS DADVAZ.DAT" begin
        ...
        if isfile(dadvaz_path)
            ...
            println(
                "  ✅ ONS DADVAZ: $(data.header.plant_count) plants in header, " *
    ...
    @testset "ONS Cross-File Consistency" begin
        ...
        if all(isfile, [entdados_path, termdat_path, operut_path, dadvaz_path])
            ...
            println(
                "     Thermal: ENTDADOS=$(entdados_thermal_count), TERMDAT=$(termdat_plant_count)",
            )
            println(
                "     Hydro: ENTDADOS=$(entdados_hydro_count), DADVAZ=$(dadvaz_plant_count)",
            )
    ...
    @testset "ONS Network Features" begin
        ...
        if isfile(desselet_path)
            ...
            println(
                "  ✅ DESSELET: $(length(data.base_cases)) base cases, $(length(data.patamares)) patamares",
            )
        end
