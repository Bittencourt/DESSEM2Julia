...
@testset "CURVTVIAG Parser Tests" begin
    ...
    @testset "Real CCEE Data - Full File Parsing" begin
        ...
        if isfile(filepath)
            ...
            curve_66_1 = filter(r -> r.codigo_usina_montante == 66 && r.codigo_elemento_jusante == 1, result.records)
            ...
            curve_83_1 = filter(r -> r.codigo_usina_montante == 83 && r.codigo_elemento_jusante == 1, result.records)
    ...
    @testset "Multiple Propagation Curves" begin
        ...
        curve1 = filter(r -> r.codigo_usina_montante == 100 && r.codigo_elemento_jusante == 10, result.records)
        @test length(curve1) == 3
        @test all(r -> r.tipo_elemento_jusante == "S", curve1)
        
        # Check second curve (plant 200 → plant 20)
        curve2 = filter(r -> r.codigo_usina_montante == 200 && r.codigo_elemento_jusante == 20, result.records)
