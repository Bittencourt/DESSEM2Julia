...
function print_plant_tree(
    ...
    println(
        prefix *
        extension *
        "   Storage: $(round(volume_min, digits=0))-$(round(volume_max, digits=0)) hm³",
    )
...
function main()
    ...
    total_capacity = sum(
        p.installed_capacity !== nothing ? p.installed_capacity : 0.0 for
        p in values(plant_map)
    )
    ...
    subsystem_names =
        Dict(1 => "Southeast/Center-West", 2 => "South", 3 => "Northeast", 4 => "North")

    for subsys in sort(collect(keys(subsystems)))
        plants_in_subsys = subsystems[subsys]
        name = get(subsystem_names, subsys, "Unknown")
        capacity_subsys = sum(
            plant_map[num].installed_capacity !== nothing ?
            plant_map[num].installed_capacity : 0.0 for num in plants_in_subsys
        )
        ...
        println("   Examples:")
        for (i, num) in
            enumerate(sort(plants_in_subsys)[1:min(3, length(plants_in_subsys))])
            plant = plant_map[num]
