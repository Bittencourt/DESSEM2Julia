...
function plot_electrical_network(case_dir::String; 
                                stage::Int=1,
    ...
    node_colors = [i <= length(topology.buses) ? 
                   subsystem_colors[topology.buses[i].subsystem] :
    ...
    if !isempty(subsystems)
        ...
        for (ss, count) in sort(collect(subsystems))
            color_name = Dict("SE"=>"Red", "S"=>"Blue", "NE"=>"Orange", "N"=>"Green")[ss]
...
if !isinteractive()
    ...
    if !isdir(case_dir)
        ...
        println()
        println("Example:")
        println("  julia examples/plot_network_simple.jl docs/Sample/DS_ONS_102025_RV2D11 1")
        exit(1)
    end
