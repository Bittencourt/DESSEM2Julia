    ...
    for (line_num, line) in enumerate(eachline(io))
        ...
        if in_area_block
            try
                ...
                @warn "Failed to parse area record at line $line_num" exception=(e, catch_backtrace())
            end
        elseif in_usina_block
            try
                ...
                @warn "Failed to parse usina record at line $line_num" exception=(e, catch_backtrace())
            end
        end
    end
...
# Convenience method
parse_areacont(filename::AbstractString) = open(io -> parse_areacont(io, filename), filename)
