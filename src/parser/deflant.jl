...
"""
function parse_stage_date(line::AbstractString, start_pos::Int)::Tuple{Union{String, Int}, Union{Int, Nothing}, Union{Int, Nothing}}
    # Day field (2 chars)
...
"""
function parse_deflant_record(line::AbstractString, filename::AbstractString, line_num::Int)::DeflantRecord
...
function parse_deflant(io::IO, filename::AbstractString)::DeflantData
    ...
    for (line_num, line) in enumerate(eachline(io))
        ...
        try
            ...
            @warn "Failed to parse DEFANT record at line $line_num" exception=(e, catch_backtrace())
            continue
        end
    end
