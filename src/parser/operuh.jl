...
function parse_rest_record(line::AbstractString)::Union{HydroConstraintREST, Nothing}
    ...
    try
        ...
        variation_type = isempty(variation_type_str) ? nothing : parse(Int, variation_type_str)
        window_duration = parse_float(strip(extract_field(line, 56, 60)), allow_blank=true)  # 55+1 to 59+1
